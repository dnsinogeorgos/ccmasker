...
time time zcat testdata/*.gz | ./ccmasker -cpuprofile=cpu.prof > /dev/null
echo 'svg > cpu.svg' | go tool pprof cpu.prof 2&> /dev/null

time time zcat testdata/*.gz | ./ccmasker -memprofile=mem.prof > /dev/null
...
time time zcat testdata/*.gz | ./ccmasker.py > /dev/null
